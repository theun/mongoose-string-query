  ...
  schema.statics.apiQueryParams = function(rawParams) {
    ...
    var parseSchemaForKey = function (schema, keyPrefix, lcKey, val, operator) {
      ...
      if (matches = lcKey.match(/(.+)\.(.+)/)) {
        ...
        || schema.paths[lcKey].constructor.name === "DocumentArray"
      ) {
        paramType = "Array";
      }
      console.log('mongoose-string-query:', lcKey, schema.paths[lcKey].constructor.name, paramType);
      ...
      if (paramType === "Boolean") {
        ...
        if (val.match(/([0-9]+)/)) {
          if (operator === "size") {
            var newParam = {};
            newParam["$" + operator] = parseInt(val);
            addSearchParam(newParam);
          } else {//changed
            addSearchParam(parseInt(val));
          }
